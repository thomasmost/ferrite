...
- **Bindgen names used below** (all present in `bindings_emery.rs` after Phase 2): `sys::tm` (**Pebble's own 11-field definition, not newlib's** — `pebble.h` redefines `struct tm` and the SDK compiles with `-D_TIME_H_`, so the generated struct is the nine standard ints plus `tm_gmtoff: c_int` and `tm_zone: [c_char; 6]`, 48 bytes; see the corrected "Codebase verified" note at the top of this file), `sys::TimeUnits::SECOND_UNIT` (newtype associated const), `sys::TickHandler` = `Option<unsafe extern "C" fn(*mut tm, TimeUnits)>`, `sys::heap_bytes_free()`. Safe `extern "C" fn`s coerce to the `unsafe extern "C" fn` pointer type when wrapped in `Some(...)`.
